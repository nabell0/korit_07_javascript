...
console.log(todos); // JS 객체가 아니라 배열이라는 점에 주목할 필요 -> 맨 처음에 localSorage를 뒤지면 빈 배열이 저장되겠죠. 이후에 배열 내부의 element들에 객체를 추가했습니다.

// Todo 리스트를 불러오는 과정이 필요합니다.
// 페이지에 처음 들어가게 됐을 때의 로직이라고 생각하면 편함.
function renderTodos() {
  ...
  todos.forEach((todo, index) => {
    ...
    // li 태그를 생성한다는 의미
    // 웬만하면 forEach()/ map() method에는 두개 이상의 argument가 요구된다고 알아두시면 속편합니다.
    ...
    if (todo.completed) { // localStorage의 'todos' key의 내부를 확인했을 때 배열로 저장되어있고, 저기 내부에 JS 객체가 있는데, 그 객체가 현재 todo라고 이름 붙여져있습니다. 그 todo들은 text라는 key, completed라는 key를 가지고 있었습니다.
    ...
    // 37번 라인부터 41번 라인까지의 해석은 js 객체 상의 key가 completed인 애의 value가 true라면 li 태그에 completed라는 속성을 추가해주는데, 이게 추가 되면 .css에서 정의된 추가 스타일을 적용하기 위한 것입니다.(취소선, 배경색 등).
    ...
    span.textContent = todo.text; // 내용이 있는거니까 배열내의 element인 JS 객체의 text라는 키의 value를 span 태그의 내부에 집어넣어서 페이지 상에서 출력
...
function saveTodos() {
  ...
  localStorage.setItem('temp','안녕') // 12번라인 때문에 항상 localStroage의 value가 배열이라고 오해살까봐 추가한 코드
}
...
function addTodo() {
  ...
  // method는 method하나 당 기능 하나라고 생각하셔야합니다.
  // 모듈화 
}

// '추가' 버튼 클릭 이벤트
addBtn.addEventListener('click', addTodo);  // button 태그에 onclick 속성. -> 두번째 argument로 addTodo 함수를 넣었는데, addTodo()가 아니라는 점에도 주목해야한다고 했습니다.
